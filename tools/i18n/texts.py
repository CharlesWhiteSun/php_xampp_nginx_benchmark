...
TEXTS = {
    "en": {
        ...
        "endpoint_cpu_usecase": "Computational analysis, data processing, analytics engines, algorithmic workloads",
        "endpoint_cpu_method": "Iterative mathematical computation with no I/O; varying CPU iteration counts to stress CPU limits",
        "endpoint_json_usecase": "REST APIs, real-time data feeds, spa backends, middleware serialization",
        "endpoint_json_method": "JSON encoding/decoding with payload size variations; tests serialization efficiency and handling capacity",
        "endpoint_io_usecase": "Database applications, file-based services, backend queries, content management systems",
        "endpoint_io_method": "Simulated file I/O and memory operations; varied block sizes and iteration counts to test concurrent I/O patterns",
        ...
        "interp_throughput_recommend": "1) Compare across endpoints to identify bottlenecks. 2) If throughput favors multi-core (NGINX), workload is parallelizable and benefits from scaling. 3) For batch processing or competitive APIs, prioritize the throughput winner.",
        ...
        "interp_latency_recommend": "1) Identify outlier endpoints; optimize those first. 2) If NGINX shows lower latency, its architecture is more efficient for this workload. 3) For interactive services (web UI, APIs), prioritize the latency winner. Aim for <100ms average for good UX.",
        ...
        "interp_percentile_recommend": "1) P99 should be <3x the average latency for consistent performance. 2) If P99 is significantly high, investigate resource contention. 3) Real-time or SLA-sensitive applications must prioritize systems with low tail latency. Monitor both P90 and P99.",
        ...
        "interp_transfer_recommend": "1) Compare across endpoints; JSON typically transfers more data than CPU-only. 2) If transfer rate is consistently lower on one system, optimize network handling. 3) For data-intensive services, ensure sufficient bandwidth margin; avoid systems near saturation.",
        ...
        "interp_consistency_recommend": "1) Favor the system with tighter P90/P99 bounds for production stability. 2) Monitor both average and percentile metrics; extremes matter for SLAs. 3) If variability is high, profile the system to find the root cause (GC pauses, thread contention, etc.).",
    ...
    "zh": {
        ...
        "performance_analysis_section": "效能分析",
        "desc_performance_analysis": "全面的視覺化分析，涵蓋所有端點的基準測試效能指標。互動式圖表展示吞吐量、延遲、資料傳輸和統計分佈。",
        "th_timestamp": "時間",
        "th_server": "服務",
        "th_endpoint": "端點",
        "th_req": "Req/sec",
        ...
        "endpoint_cpu_usecase": "運算分析、資料處理、分析引擎、演算法工作負載",
        "endpoint_cpu_method": "無 I/O 的迭代數學運算；改變 CPU 迴圈次數來壓測 CPU 極限",
        "endpoint_json_usecase": "REST API、即時數據饋送、SPA 後端、中介軟體序列化",
        "endpoint_json_method": "JSON 編碼/解碼與 payload 大小變異；測試序列化效率與處理容量",
        "endpoint_io_usecase": "資料庫應用、檔案型服務、後端查詢、內容管理系統",
        "endpoint_io_method": "模擬檔案 I/O 與記憶體操作；變異區塊大小和迴圈次數以測試併發 I/O 模式",
        "interp_metric": "效能指標",
        ...
        "interp_throughput_recommend": "1) 跨端點比較以找出瓶頸。2) 若多核心吞吐量領先，工作負載可並行化且受益於擴展。3) 批處理或高競爭 API，優先選吞吐勝出的方案。",
        "interp_latency": "回應延遲 (平均 ms)",
        "interp_latency_eval": "越低越好；代表典型使用者體驗。延遲高代表處理延遲或資源競爭。影響互動應用的回應速度。",
        "interp_latency_recommend": "1) 找出異常端點並優先優化。2) 若 NGINX 延遲更低，其架構對此工作負載更高效。3) 互動式服務（網頁 UI、API），優先選延遲勝出者。目標 <100ms 平均延遲以提供良好體驗。",
        "interp_percentile": "尾端延遲 (P90, P99 ms)",
        "interp_percentile_eval": "對最壞情況預測至關重要；P99 反映最慢 1% 的請求。P99 偏高代表效能不均，可能導致使用者不滿。",
        "interp_percentile_recommend": "1) P99 應 <3x 平均延遲以維持一致性能。2) P99 明顯偏高時，調查資源競爭。3) 即時或 SLA 敏感應用必須優先選擇尾端延遲更低的方案。同時監控 P90 與 P99。",
        "interp_transfer": "資料傳輸速率 (KB/sec)",
        "interp_transfer_eval": "KB/sec 吞吐量越高表示網路/I/O 效率越好。對 payload 密集端點或頻寬受限情境尤其有用。",
        "interp_transfer_recommend": "1) 跨端點比較；JSON 通常比 CPU-only 傳輸更多資料。2) 若某系統傳輸速率持續偏低，優化網路処理。3) 資料密集服務需確保充分頻寬裕度；避免系統飽和。",
        "interp_consistency": "效能一致性與穩定性",
        "interp_consistency_eval": "比較 P50/P90/P99 在端點間的變異。若某系統效能一致，更可預測；若另一系統不規律，可能存在每request 開銷或資源競爭。",
        "interp_consistency_recommend": "1) 優先採用 P90/P99 邊界更緊密的系統以提高生產穩定性。2) 同時監控平均與分位數指標；極值對 SLA 影響大。3) 變異性高時，分析系統找出根本原因（GC 停頓、執行緒競爭等）。",
